...
// Fight / cinematics (wide 16:9 for four-corner group battles)
...
if (!HOUSE_MNEMONIC) {
  throw new Error("Set DC_WALLET or HOUSE_MNEMONIC in your environment before running darkGroupFight.js")
}
...
function makeYouTubeMetadataGroupBattle({
  names = [],
  winnerName = null,
  durationSec,
}) {
  const cleanNames = names.map((name) => String(name || '').trim()).filter(Boolean)
  const titleNames = cleanNames.join(' vs ')
  const title = `Dark Coin arena 4-way battle: ${titleNames}`.slice(0, 100)

  const lines = []
  lines.push(`${titleNames} - four blockchain champions enter the Dark Coin arena.`)
  if (winnerName) {
...
function applyChampionSkillAfterStatusEvent({ event, actor, target, category = null, moveKind = null }) {
  ...
  if (event?.resisted) {
    ...
    if (shieldOnResist) {
      applyChampionSkillStatus({ actor: resolvedTarget, target: resolvedTarget, effectKey: 'shield', amount: shieldOnResist, isBuffApplication: true, events: skillEvents })
    }
    const hastenOnEvade = getChampionSkillLevel(resolvedTarget, 2002)
    if (hastenOnEvade) {
      applyChampionSkillStatus({ actor: resolvedTarget, target: resolvedTarget, effectKey: 'hasten', amount: hastenOnEvade, isBuffApplication: true, events: skillEvents })
    }
    return skillEvents
  }
...
function getChampionSkillDamageBonus({ actor, target, moveEffectKey }) {
  ...
  if (conductivity && String(moveEffectKey || '').toLowerCase() === 'paralyze' && getStack(getChampionEffectsBucket(target), 'drown') > 0) {
    return conductivity
  }
  return 0
}
...
const ARENA_FIGHT_TYPE_LABEL = 'GROUP FIGHT'
...
function getRandomLiveOpponent(fighters, actor) {
  const options = fighters.filter((fighter) => fighter.alive && fighter.side !== actor.side)
...
async function createGroupFightFrames({
  fighters,
  backgroundPath,
  outFramesDir,
  ...
  if (!Array.isArray(fighters) || fighters.length !== 4) {
    throw new Error(`createGroupFightFrames expected exactly 4 fighters; got ${fighters?.length || 0}`)
  }
  ...
  function applyMoveEffectStacks({ meta, moveKind, actor, target, effectAmountMultiplier = 1 }) {
    ...
    const effectTarget = moveKind === 'buff' ? actor : target
  ...
  async function animateMove({ actor, target, move, moveKind }) {
    ...
    if (moveKind === 'buff') {
      ...
      for (let i = 0; i < moveDuration; i += 1) {
        ...
        const x = actor.x + Math.floor(actor.idleImg.bitmap.width / 2) - Math.floor(currentVisualImg.bitmap.width / 2)
        const y = actor.y - currentVisualImg.bitmap.height - Math.round(actor.idleImg.bitmap.height * 0.28 * t)
        const frame = bg.clone()
        await drawGroupBarsAndEffects(frame)
  ...
  async function resolveMove({ actor, target, move, moveKind, impactTimeSec }) {
    ...
    if (moveKind === 'buff') {
      ...
      const { dmg: healAmountRaw } = calcDamageRPG({
        ...
        defenderStats: actor.stats,
        attackerEffects: actor.effects,
        defenderEffects: actor.effects,
        arenaMap,
      })
      const prev = actor.hp
      const healAmount = Math.max(0, Math.round(healAmountRaw * buffCritMultiplier))
      const next = Math.min(actor.maxHp, actor.hp + healAmount)
      if (next > prev) {
        await animateHealthChange({
          target: actor,
          fromHp: prev,
      ...
      await animateStatusEventWithSkillTriggers(
        applyMoveEffectStacks({ meta, moveKind, actor, target: actor, effectAmountMultiplier: buffCritMultiplier }),
        { actor, target: actor, category: meta.type, moveKind }
      )
      await animateSkillStatusEvents(applyChampionSkillAfterBuffMove({ actor, target: actor, category: meta.type }))
      return
    }
  ...
  const resolveArenaEndGameAfterAction = async () => {
    ...
    for (const fighter of liveFighters().filter((candidate) => candidate.side !== winnerFighter.side)) {
      audioTimeline?.push('death', fIdx / FIGHT_FPS)
      await eliminateFighter(fighter)
    }
    return true
  ...
  let turnGuard = 0
  while (liveCount() > 1 && turnGuard < 1000) {
    turnGuard += 1
    while (liveCount() > 1 && !liveFighters().some((fighter) => fighter.cd <= 0)) {
      tickCooldowns()
      await drawBattleFrame()
    }
    ...
    if (!actor.alive || liveCount() <= 1) continue

    const move = pickMoveFor(actor)
    const moveKind = getExplicitMoveKind(move.meta)
    const target = moveKind === 'buff' ? actor : getRandomLiveOpponent(fighters, actor)
    if (!target) break
    ...
    for (let i = 0; i < 6 && liveCount() > 1; i += 1) {
      tickCooldowns()
      await drawBattleFrame()
    }
  }
  ...
  if (turnGuard >= 1000) {
    console.warn('Group battle reached turn guard; choosing highest HP survivor.')
    const survivors = liveFighters().sort((a, b) => b.hp - a.hp)
    for (const fighter of survivors.slice(1)) await eliminateFighter(fighter)
  }

  const winner = liveFighters()[0] || fighters.sort((a, b) => b.hp - a.hp)[0]
  await animateVictory(winner)
  ...
  return {
    ...
    loserSides: fighters.filter((fighter) => fighter.side !== winner.side).map((fighter) => fighter.side),
    loserAssetIds: fighters
      .filter((fighter) => fighter.side !== winner.side)
...
const ACTIVE_BATTLE_CHAMPION_SOURCE = 'darkGroupFight.js'
const RUN_INSTANCE_ID = `darkGroupFight_${Date.now()}_${Math.random().toString(16).slice(2, 10)}`
...
// Generate a wide four-corner arena background using OpenAI images,
...
async function generateFightBackground(aName, bName, outDir, arenaMap = null, standingLayoutHint = '') {
  ...
  const prompt = `
Old-school pixel-art style dark fantasy four-corner arena for map: ${mapName}.
Terrain theme: ${terrain}
Wide 16:9 scene, viewed from a slight top-down three-quarter camera angle.
Keep the same readable group battle structure every time: one continuous arena floor / fight-pit tile surface that covers the entire image edge to edge, with no empty void, no distant backdrop, and no separate floating platform.
...
function buildGroupFighterConfig(character) {
  return {
...
async function mainOnce() {
  ...
  const claimedFight = await pickAndReserveArenaChampionGroup(validArena)
  const stopReservationHeartbeat = startActiveBattleChampionHeartbeat(claimedFight)
  ...
  try {
    ...
    console.log('Using arena group characters:')
    characters.forEach((character, index) => {
      console.log(
        `  ${GROUP_SIDES[index]} = ${character.creature_name} (assetId ${character.arena.assetId})`
      )
    })
    ...
    const fightDir = path.join(OUT_DIR, 'group_fight_scene')
    ensureDir(fightDir)
    await emptyDir(fightDir)
    ...
    const outcome = await createGroupFightFrames({
      fighters: characters.map(buildGroupFighterConfig),
      backgroundPath: bgPath,
      outFramesDir: framesDir,
      scaleFraction: GROUP_FIGHT_SCALE_FRACTION,
      audioTimeline,
    ...
    console.log(`Group fight frames created: ${outcome.frames} frames at ${FIGHT_FPS} fps`)

    const silentVideo = path.join(fightDir, 'group_character_battle.mp4')
    await stitchFramesToVideo(framesDir, silentVideo, FIGHT_FPS)
    ...
    try {
      const winnerAssetId = outcome.winnerAssetId
      const loserAssetIds = outcome.loserAssetIds || []

      console.log('\nGroup fight outcome:')
      console.log(`  Winner side: ${outcome.winnerSide}`)
      console.log(`  Winner name: ${outcome.winnerName}`)
      console.log(`  Winner assetId: ${winnerAssetId}`)
      console.log(`  Loser assetIds: ${loserAssetIds.join(', ')}`)

      if (winnerAssetId) {
        if (loserAssetIds.length !== 3) {
          throw new Error(
            `Cannot submit group reward without exactly 3 loser asset ids; found ${loserAssetIds.length}.`
          )
        }
        const winHolders = await findAssetHolders(winnerAssetId, { minAmount: 1, maxAccounts: 1 })
        if (!winHolders.length) console.log('  Winner holder(s): none found')
        else {
          const { txId, confirmedTxn } = await sendGroupedTxnsWithFreshParamsAndRetry({
            client,
            houseAccount,
            ...
            buildTxns: async (params) => {
              ...
              txns.push(
                algosdk.makeApplicationNoOpTxnFromObject({
                  ...
                  accounts: [winHolders[0].address],
                  foreignApps: [],
                  foreignAssets: [winnerAssetId, ...loserAssetIds, 1088771340],
                  boxes: [],
                })
              )
          ...
          console.log('Submitted group reward txId:', txId)
          console.log(confirmedTxn)
        }
      }
    } catch (e) {
    ...
    const slugifyStorage = (s) =>
      String(s || '')
        ...
        .slice(0, 80)
    const shortId = () => `${Date.now()}_${Math.random().toString(16).slice(2, 10)}`
    const meta = makeYouTubeMetadataGroupBattle({
      names,
    ...
    let storageUpload = null
    try {
      const objectPath = `group_fights/${slugifyStorage(names.join('_vs_'))}_${shortId()}.mp4`
      const videoBytes = await fsp.readFile(finalVideo)
      const storage = getStorage()
      const objRef = storageRef(storage, objectPath)
      const customMetadata = {
      ...
      console.log('\nUploading group fight video to Firebase Storage...')
      const snap = await uploadBytes(objRef, videoBytes, {
        contentType: 'video/mp4',
        cacheControl: 'public, max-age=31536000',
        customMetadata,
    ...
    try {
      ...
      const thumbObjectPath = `group_fights/thumbnails/${slugifyStorage(names.join('_vs_'))}_${shortId()}.png`
      const thumbBytes = await fsp.readFile(thumbLocalPath)
      const storage = getStorage()
      const thumbRef = storageRef(storage, thumbObjectPath)
      const thumbSnap = await uploadBytes(thumbRef, thumbBytes, {
    ...
    if (!announcementUrl) {
      ...
      try {
        await sendDiscordFightAnnouncement({
          ...
          aName: names.join(' vs '),
          bName: '',
          matchupText: names.map((name) => `**${name}**`).join(' vs '),
          youtubeUrl,
          firebaseVideoUrl,
          thumbnailUrl: thumbnailDownloadURL,
          youtubeUploadSucceeded,
...
async function runOnceAndExit() {
  try {
    await mainOnce()
    console.log('Group fight simulation completed.')
  } catch (err) {
    console.error('Group fight simulation failed:', err?.stack || err?.message || err)
    process.exitCode = 1
  }
}
