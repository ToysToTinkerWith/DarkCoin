...
if (!HOUSE_MNEMONIC) {
  throw new Error("Set DC_WALLET or HOUSE_MNEMONIC in your environment before running darkFight.js")
}
...
const ARENA_MAP_INFO_END_GAME_ROUND = 50
...
function exponentialStackEndGame(effect, displayName) {
  return {
    ...
    description: `Starting on the endgame round, both champions gain exponentially increasing ${effect} stacks after each round until the match ends.`,
    roundStackFormula: `2 ^ (currentRound - endgameRound) ${effect} stacks applied to each champion after the round resolves.`,
    winnerCriteria: [
...
function applyChampionSkillStatus({
  actor,
  target,
  effectKey,
  amount,
  ...
  if (normalizedEffectKey === 'cleanse') {
    ...
    const event = {
      ...
      target,
      targetSide: getChampionSide(target),
      resisted: false,
      skillTreeEffect: true,
  ...
  const isNegativeApplication =
    ...
    actor &&
    target &&
    !isSameChampionSide(actor, target)
...
function applyChampionSkillAfterStatusEvent({ event, actor, target, category = null, moveKind = null }) {
  ...
  if (event?.resisted) {
    ...
    if (shieldOnResist) {
      applyChampionSkillStatus({
        actor: resolvedTarget,
        target: resolvedTarget,
        effectKey: 'shield',
        amount: shieldOnResist,
  ...
  if (effectName === 'bleed') {
    ...
    if (drownInBlood) {
      applyChampionSkillStatus({ actor: applier, target: resolvedTarget, effectKey: 'drown', amount: drownInBlood, events: skillEvents })
    }
  }
  ...
  if (effectName === 'burn') {
    ...
    if (fieryHell) {
      applyChampionSkillStatus({ actor: applier, target: resolvedTarget, effectKey: 'doom', amount: fieryHell, events: skillEvents })
    }
  }
  ...
  if (effectName === 'slow' && selfApplication) {
    ...
    if (slowMotion) {
      applyChampionSkillStatus({ actor: resolvedTarget, target: resolvedTarget, effectKey: 'focus', amount: slowMotion, isBuffApplication: true, events: skillEvents })
    }
  }
  ...
  if (effectName === 'shield') {
    ...
    if (paladin) {
      applyChampionSkillStatus({ actor: applier, target: resolvedTarget, effectKey: 'bless', amount: paladin, isBuffApplication: true, events: skillEvents })
    }
    const bulwark = selfApplication ? getChampionSkillLevel(resolvedTarget, 1802) : 0
    if (bulwark) {
      applyChampionSkillStatus({ actor: resolvedTarget, target: resolvedTarget, effectKey: 'bless', amount: bulwark, isBuffApplication: true, events: skillEvents })
    }
  }
  ...
  if (effectName === 'strengthen') {
    ...
    if (battleTempo) {
      applyChampionSkillStatus({ actor: applier, target: resolvedTarget, effectKey: 'hasten', amount: battleTempo, isBuffApplication: true, events: skillEvents })
    }
  }
  ...
  if (effectName === 'bless' && selfApplication) {
    ...
    if (faith) {
      applyChampionSkillStatus({ actor: resolvedTarget, target: resolvedTarget, effectKey: 'focus', amount: faith, isBuffApplication: true, events: skillEvents })
    }
  }
  ...
  if (effectName === 'cleanse') {
    ...
    if (judgement) {
      applyChampionSkillStatus({ actor: applier, target: resolvedTarget, effectKey: 'empower', amount: judgement, isBuffApplication: true, events: skillEvents })
    }
    const fastRecovery = selfApplication ? getChampionSkillLevel(resolvedTarget, 1702) : 0
    if (fastRecovery) {
      applyChampionSkillStatus({ actor: resolvedTarget, target: resolvedTarget, effectKey: 'hasten', amount: fastRecovery, isBuffApplication: true, events: skillEvents })
    }
  }
  ...
  if (
    actor &&
    !selfApplication &&
    NEGATIVE_EFFECT_KEYS.has(effectName) &&
    !event?.resisted
  ) {
    const mindWard = getChampionSkillLevel(resolvedTarget, 2102)
    if (mindWard) {
      applyChampionSkillStatus({ actor: resolvedTarget, target: resolvedTarget, effectKey: 'bless', amount: mindWard, isBuffApplication: true, events: skillEvents })
    }
  }
...
function applyChampionSkillAfterBuffMove({ actor, target, category = null }) {
  ...
  if (greenAura) {
    applyChampionSkillStatus({ actor, target: buffTarget, effectKey: 'nurture', amount: greenAura, isBuffApplication: true, events })
  }

  if (isMelee(category)) {
    const honestTouch = getChampionSkillLevel(actor, 1102)
    if (honestTouch) {
      applyChampionSkillStatus({ actor, target: buffTarget, effectKey: 'empower', amount: honestTouch * 2, isBuffApplication: true, events })
    }
    const washedHands = getChampionSkillLevel(actor, 1501)
    if (washedHands) {
      applyChampionSkillStatus({ actor, target: buffTarget, effectKey: 'cleanse', amount: washedHands, isBuffApplication: true, events })
    }
  }

  if (isMagic(category)) {
    const miracle = getChampionSkillLevel(actor, 1302)
    if (miracle && Math.random() < Math.min(1, miracle * 0.25)) {
      applyChampionSkillStatus({ actor, target: actor, effectKey: 'bless', amount: 4, isBuffApplication: true, events })
    }
  }
...
function applyChampionSkillAfterMoveHit({
  actor,
  target,
  category = null,
  moveKind = null,
  ...
  if (moveKind === 'curse') {
    ...
    if (toxicCask) {
      applyChampionSkillStatus({ actor, target, effectKey: 'poison', amount: toxicCask, events })
    }
  }
  ...
  if (isMelee(category)) {
    ...
    if (armDay) {
      applyChampionSkillStatus({ actor, target: actor, effectKey: 'strengthen', amount: armDay, isBuffApplication: true, events })
    }
  ...
  if (isRanged(category)) {
    ...
    if (arrowOfTruth) {
      applyChampionSkillStatus({ actor, target: actor, effectKey: 'empower', amount: arrowOfTruth, isBuffApplication: true, events })
    }
    const preciseAim = getChampionSkillLevel(actor, 2001)
    if (preciseAim) {
      applyChampionSkillStatus({ actor, target: actor, effectKey: 'focus', amount: preciseAim, isBuffApplication: true, events })
    }
  }
  ...
  if (isMagic(category)) {
    ...
    if (arcaneStudy) {
      applyChampionSkillStatus({ actor, target: actor, effectKey: 'empower', amount: arcaneStudy, isBuffApplication: true, events })
    }
  }
...
function applyChampionSkillAfterMiss({ actor }) {
  ...
  if (quickstep) {
    applyChampionSkillStatus({ actor, target: actor, effectKey: 'hasten', amount: quickstep, isBuffApplication: true, events })
  }
  return events
}
...
function applyChampionSkillAfterHealthChange({ target, previousHp, nextHp, isHealing = false }) {
  ...
  if (
    ironConstitution &&
    runtime &&
    !runtime.triggered.halfHealthShield &&
    safeNumber(previousHp, 0) > halfHp &&
    safeNumber(nextHp, 0) <= halfHp
  ) {
  ...
  if (isHealing && secondWind && safeNumber(previousHp, 0) < halfHp && safeNumber(nextHp, 0) > safeNumber(previousHp, 0)) {
    applyChampionSkillStatus({ actor: target, target, effectKey: 'nurture', amount: secondWind, isBuffApplication: true, events })
  }
...
function getChampionSkillDamageBonus({ actor, target, moveEffectKey }) {
  ...
  if (
    conductivity &&
    String(moveEffectKey || '').toLowerCase() === 'paralyze' &&
    getStack(getChampionEffectsBucket(target), 'drown') > 0
  ) {
    return conductivity
  }
  return 0
...
const IMAGE_READ_RETRY_ATTEMPTS = 5
const IMAGE_READ_RETRY_BASE_MS = 750

function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms))
}

function isRetryableImageReadError(error) {
  const message = String(error?.message || error || '')
  return (
    message.includes('HTTP Status 408') ||
    message.includes('HTTP Status 425') ||
...
async function readJimpWithRetry(source, label = 'image') {
  let lastError = null

  for (let attempt = 1; attempt <= IMAGE_READ_RETRY_ATTEMPTS; attempt += 1) {
    try {
      return await Jimp.read(source)
    } catch (error) {
      lastError = error
      const retryable = isRetryableImageReadError(error)
      if (!retryable || attempt >= IMAGE_READ_RETRY_ATTEMPTS) break

      const delayMs =
        IMAGE_READ_RETRY_BASE_MS * 2 ** (attempt - 1) +
        Math.floor(Math.random() * 250)
      console.warn(
        `Image read failed for ${label} (attempt ${attempt}/${IMAGE_READ_RETRY_ATTEMPTS}); retrying in ${delayMs}ms:`,
        error?.message || error
      )
      await sleep(delayMs)
    }
...
async function downloadImageSequence(urls, outDir, prefix) {
  ...
  for (let i = 0; i < urls.length; i += 1) {
    ...
    const img = await readJimpWithRetry(url, `${prefix}[${i}]`)
...
async function readAndScaleFrameSequence(paths, targetH) {
  ...
  for (const p of Array.isArray(paths) ? paths : []) {
    try {
      const img = await readJimpWithRetry(p, path.basename(p))
...
async function blinkTargetFrames({
  ...
  for (let i = 0; i < frames; i++) {
    ...
    if (overlayFrame) await overlayFrame(frame)
...
async function zoomStatsSequence({
  ...
  const doZoomStep = async (z, recordStart = false) => {
    ...
    if (overlayFrame) await overlayFrame(cropped)
...
async function victorySequence({
  ...
  for (let i = 0; i < VICTORY_BANNER_FRAMES; i++) {
    ...
    const by = Math.max(VICTORY_BANNER_TOP, 144)
    const frame = cropped
...
const arenaMapTitleFontBlack = await loadFontBuiltin(32, 'black')
  ...
  const drawPersistentMapHud = async (frame, options = {}) => {
    if (suppressMapHud) return
    drawArenaMapInfoPanel(frame, arenaMap, {
      font: arenaMapFont,
      ...
      bgW,
      y: options.y ?? getArenaMapInfoTopY(),
      compact: false,
  ...
  const buildFrameBEmote = async () => {
    ...
    await drawPersistentMapHud(frame)
  }
  ...
  async function applyOngoingEffectsForSide(side, frameIndexForAudio) {
    const isA = side === 'A'
    const champion = getDuelChampionBySide(side)
    await animateDuelSkillStatusEvents(applyChampionSkillBeforeTurn({ actor: champion }))
    const effects = isA ? A_effectTotals : B_effectTotals
    const delta =
      computeOngoingEffectHpDelta(effects, arenaMap) +
      getChampionSkillOngoingHpBonus(champion)
    if (!delta) return
    ...
    if (delta < 0) {
      ...
      fIdx = await animateHealthDrop({
        ...
        aStats,
        bStats,
        overlayFrame: drawPersistentMapHud,
      })
    } else {
    ...
    await animateDuelSkillStatusEvents(applyChampionSkillAfterHealthChange({
      target: champion,
      previousHp: prevHp,
      nextHp: newHp,
      isHealing: delta > 0,
  ...
  const duelChampionA = {
    ...
    stats: aStats,
    charObj: aCharObj,
    skillTreeRuntime: aCharObj?.skillTreeRuntime,
    get hp() { return A_HP },
    set hp(value) { A_HP = value },
    get maxHp() { return A_MAX },
  }
  const duelChampionB = {
    side: 'B',
    effects: B_effectTotals,
    stats: bStats,
    charObj: bCharObj,
    skillTreeRuntime: bCharObj?.skillTreeRuntime,
    get hp() { return B_HP },
    set hp(value) { B_HP = value },
    get maxHp() { return B_MAX },
  }
  const getDuelChampionBySide = (side) => (side === 'A' ? duelChampionA : duelChampionB)

  async function animateDuelSkillStatusEvents(events) {
    const normalizedEvents = (events || [])
      .filter(Boolean)
      .map((event) => ({
        ...event,
        targetSide: event.targetSide || getChampionSide(event.target),
      }))
  ...
  async function animateDuelStatusEventWithSkillTriggers(event, context = {}) {
    if (!event) return
    await animateDuelSkillStatusEvents([event])
    const target = context.target || getDuelChampionBySide(event.targetSide)
    const actor = context.actor || (event.actorSide ? getDuelChampionBySide(event.actorSide) : null)
    await animateDuelSkillStatusEvents(applyChampionSkillAfterStatusEvent({
      event,
      actor,
      target,
...
if (startItemEffectEvents.length) {
  for (const event of startItemEffectEvents) {
    await animateDuelStatusEventWithSkillTriggers(event, {
      actor: event?.actorSide ? getDuelChampionBySide(event.actorSide) : null,
      target: getDuelChampionBySide(event?.targetSide),
      moveKind: event?.targetSide === event?.actorSide ? 'buff' : 'curse',
    })
  }
  recomputeCooldownClocks()
}
  ...
  const getCurrentBattleFrame = async () => {
    const frame = bg.clone()
    frame.composite(getAIdleFrame(fIdx), axFinalX, aY)
    frame.composite(getBIdleFrame(fIdx), bxFinalX, bY)
    await drawBarsAndEffects(frame, cdA, cdB, A_HP, A_MAX, B_HP, B_MAX)
  ...
  const announceArenaEndGame = async () => {
    ...
    try {
      fIdx = await animateArenaMapInfoZoom({
        baseFrameBuilder: getCurrentBattleFrame,
        arenaMap,
        outFramesDir,
        framesSoFar: fIdx,
        bgW,
  ...
  const applyArenaCascadeIfNeeded = async () => {
    ...
    if (battleRound < getArenaMapEndGameRound(arenaMap)) return

    const effectKey = String(endGame.resolution.appliedEffect || arenaMap?.effect || '').toLowerCase()
    if (!effectKey) return
    const stacks = Math.min(9999, Math.max(1, 2 ** Math.max(0, battleRound - getArenaMapEndGameRound(arenaMap))))

    A_effectTotals[effectKey] = getStack(A_effectTotals, effectKey) + stacks
    B_effectTotals[effectKey] = getStack(B_effectTotals, effectKey) + stacks
  ...
  const resolveArenaEndGameAfterAction = async () => {
    const endGame = getArenaMapEndGame(arenaMap)
    if (!endGame || battleRound !== getArenaMapEndGameRound(arenaMap)) return false

    await announceArenaEndGame()
    if (endGame.resolution?.mode !== 'decideAtRoundStart') return false

    const winnerContestant = pickArenaMapWinner(buildArenaContestants(), arenaMap)
    if (!winnerContestant) return false
    if (winnerContestant.side === 'A') B_HP = 0
    else A_HP = 0
    if (audioTimeline) audioTimeline.push('death', fIdx / FIGHT_FPS)
    return true
  }
...
let impactTimeSec = null
      ...
      const actorChampion = isA ? duelChampionA : duelChampionB
    ...
    // Heal for the damage this move would deal (same damage formula, with stats/effects)
      ...
      if (isA) {
        ...
        await animateDuelStatusEventWithSkillTriggers(effEv, {
          actor: duelChampionA,
          target: getDuelChampionBySide(effEv?.targetSide),
          category,
          moveKind,
        })
        await animateDuelSkillStatusEvents(applyChampionSkillAfterBuffMove({
          actor: duelChampionA,
          target: duelChampionA,
          category,
        }))

        if (false && effEv && effEv.effectName) {
    ...
    if (!buff) {
      const actorChampion = isA ? duelChampionA : duelChampionB
      const targetChampion = isA ? duelChampionB : duelChampionA
      await animateDuelSkillStatusEvents(applyChampionSkillBeforeDamagingMove({ actor: actorChampion }))
      ...
      if (hits) {
        ...
        const targetHadBleed = getStack(targetChampion.effects, 'bleed') > 0
        const targetHadBurn = getStack(targetChampion.effects, 'burn') > 0
        const targetHadSlow = getStack(targetChampion.effects, 'slow') > 0
        const isCritical = rollChampionSkillCritical(actorChampion, arenaMap)
        ...
        for (const event of itemHitEvents) {
          await animateDuelStatusEventWithSkillTriggers(event, {
            actor: actorChampion,
            target: getDuelChampionBySide(event?.targetSide),
            category,
        ...
        await animateDuelSkillStatusEvents(applyChampionSkillAfterMoveHit({
          actor: actorChampion,
        ...
        await animateDuelSkillStatusEvents(applyChampionSkillAfterMiss({ actor: actorChampion }))

        const arenaMissBonus = getArenaMapMissBonus(arenaMap)
        if (arenaMissBonus) {
          const actorCharObj = isA ? aCharObj : bCharObj
          const missEvent = applyStatusAmountToBucket({
            effectKey: arenaMissBonus.effectKey,
            amount: getMapPassiveAppliedAmount(
              arenaMissBonus.amount,
              arenaMissBonus.effectKey,
              actorCharObj
            ),
            targetSide: isA ? 'A' : 'B',
            actorSide: isA ? 'A' : 'B',
          ...
          await animateDuelStatusEventWithSkillTriggers(missEvent, {
            actor: actorChampion,
            target: getDuelChampionBySide(missEvent?.targetSide),
            category,
            moveKind,
          })
          if (false && missEvent && missEvent.effectName && !missEvent.resisted && missEvent.amount > 0) {
            fIdx = await animateEffectPopup({
              targetSide: missEvent.targetSide,
              effectName: missEvent.effectName,
              amount: missEvent.amount,
              framesSoFar: fIdx,
              cdA,
              cdB,
              A_HP,
    ...
    if (await resolveArenaEndGameAfterAction()) break duel_loop
...
// Shared by darkFight.js and darkGroupFight.js. Each document means a
...
const ACTIVE_BATTLE_CHAMPION_SOURCE = 'darkFight.js'
const RUN_INSTANCE_ID = `darkFight_${Date.now()}_${Math.random().toString(16).slice(2, 10)}`
...
async function prepareArenaCharacter(arenaRecord, indexLabel) {
  ...
  // Standing / idle
  const idleImg = await readJimpWithRetry(charObj.standingUrl, `${name} standing`)
  ...
  const emoteImg = await readJimpWithRetry(emoteUrl, `${name} emote`)
  await emoteImg.writeAsync(emotePath)

  const moveACharImg = await readJimpWithRetry(moveACharU, `${name} moveA character`)
  await moveACharImg.writeAsync(moveASpritePath)

  const moveBCharImg = await readJimpWithRetry(moveBCharU, `${name} moveB character`)
  await moveBCharImg.writeAsync(moveBSpritePath)

  const moveCCharImg = await readJimpWithRetry(moveCCharU, `${name} moveC character`)
  ...
  const moveAEffectImg = await readJimpWithRetry(moveAEffectUrl, `${name} moveA effect`)
  await moveAEffectImg.writeAsync(moveAEffectPath)

  const moveBEffectImg = await readJimpWithRetry(moveBEffectUrl, `${name} moveB effect`)
  await moveBEffectImg.writeAsync(moveBEffectPath)

  const moveCEffectImg = await readJimpWithRetry(moveCEffectUrl, `${name} moveC effect`)
...
async function buildDuelStandingLayoutHint(characterA, characterB) {
  const { W, H } = parseSize(FIGHT_BG_SIZE)
  const targetH = Math.max(64, Math.round(H * FIGHT_SCALE_FRACTION * 2))
  const [aIdle, bIdle] = await Promise.all([
    Jimp.read(characterA.spriteIdlePath),
    Jimp.read(characterB.spriteIdlePath),
  ])
  aIdle.resize(Jimp.AUTO, targetH, Jimp.RESIZE_BILINEAR)
  bIdle.resize(Jimp.AUTO, targetH, Jimp.RESIZE_BILINEAR)

  const centerY = Math.floor(H * 0.72)
  const edgeMargin = Math.max(16, Math.round(W * 0.08))
  const axFinalX = edgeMargin + 30
  const bxFinalX = W - edgeMargin - bIdle.bitmap.width - 30
  const anchors = [
    {
      label: 'left fighter',
      x: Math.round(axFinalX + aIdle.bitmap.width / 2),
      y: centerY,
    },
    {
      label: 'right fighter',
      x: Math.round(bxFinalX + bIdle.bitmap.width / 2),
      y: centerY,
    },
  ]
  ...
  const anchorText = anchors
    .map((anchor) => {
      ...
      return `${anchor.label} feet at x=${anchor.x}px y=${anchor.y}px (${pctX}% across, ${pctY}% down)`
    })
    .join('; ')
  ...
  return `Champion standing anchors in the final ${W}x${H} frame: ${anchorText}. Center the main platform's top walking surface around the average foot-anchor point x=${footCenterX}px y=${footCenterY}px (${footCenterPctX}% across, ${footCenterPctY}% down), because this is where the bottoms of the idle sprites land. The platform must visibly pass directly under both feet anchors, with enough solid floor around each anchor for the full sprite to stand comfortably. Keep platform edges, holes, pits, cliffs, and decorative rims away from those foot positions.`
}
...
async function generateFightBackground(aName, bName, outDir, arenaMap = null, standingLayoutHint = '') {
  ...
  const terrain = arenaMap?.terrain || 'A dark fantasy medieval arena platform.'
  const standingLayoutText = standingLayoutHint
    ? `\n${standingLayoutHint}\n`
    : ''

  const prompt = `
Old-school pixel-art style medieval arena map: ${mapName}.
Terrain theme: ${terrain}
Keep the same readable duel arena structure every time: a vertical scene with one large solid battle platform covering the bottom half of the image, clear perspective, and a grounded surface where both champions can visibly stand.
The platform must remain wide, stable, and readable, with a distinct top surface, subtle rim/edge detail, and open empty standing space on the left and right.
The visual center of the platform's usable top floor should align with the bottom of the champions' idle positions, not the center of the full image.
${standingLayoutText}
Theme the platform materials, floor markings, lighting, cracks, debris, and surrounding atmosphere to match the selected map terrain and passive effect.
...
Do not replace the platform with a flat abstract texture, empty void, distant landscape, or decorative background that makes the fighters look ungrounded.
Dark fantasy lighting, environmental details that clearly communicate the map theme, but absolutely no characters,
no people, no creatures anywhere in the scene.
No UI and no other text besides the exact map name at the bottom center.
Mood: dark fantasy, Dark Coin arena.
`

  try {
    // Generate at the same aspect ratio as the final frame so standing anchors stay aligned.
...
async function sendDiscordFightAnnouncement({
  ...
  const embed = {
    title: `⚔️ Dark Coin Arena Duel`,
    description: `**${aName}** vs **${bName}**\n\nWho will win? Watch now!${fallbackNote}`,
    color: 0x1a0a3c,
...
try {
  ...
  const standingLayoutHint = await buildDuelStandingLayoutHint(A, B)
...
async function runOnceAndExit() {
  try {
    await mainOnce()
    console.log('Dark fight simulation completed.')
  } catch (err) {
    console.error('Dark fight simulation failed:', err?.stack || err?.message || err)
    process.exitCode = 1
  }
}
