...
def approval_program():
    ...
    fight = Seq(
        ...
        Assert(Gtxn[1].asset_amount() == App.globalGet(Itob(Txn.assets[0]))),
        ...
        oppAssetBalance := AssetHolding.balance(Txn.accounts[1], Txn.assets[0]),
        Assert(oppAssetBalance.value() == Int(1)),
        If(
            ...
            Seq(
                ...
                InnerTxnBuilder.SetFields({
                    ...
                    TxnField.asset_amount: Mul(App.globalGet(Itob(Txn.assets[0])), Int(2)),
                }),
                InnerTxnBuilder.Submit(),
                App.box_put(Concat(Bytes("Battle"), Itob(App.globalGet(Bytes("battleNum")))), Txn.application_args[1]),
            ...
            Seq(
                ...
                InnerTxnBuilder.Submit(),
                App.box_put(Concat(Bytes("Battle"), Itob(App.globalGet(Bytes("battleNum")))), Txn.application_args[2]),
                App.globalDel(Itob(Txn.assets[0])),
                App.globalPut(Bytes("battleNum"), Add(App.globalGet(Bytes("battleNum")), Int(1)))
    ...
    startFight = Seq(
        ...
        Assert(assetCreator.value() == Addr("L6VIKAHGH4D7XNH3CYCWKWWOHYPS3WYQM6HMIPNBVSYZWPNQ6OTS5VERQY")),
        If(App.globalGet(Itob(Txn.assets[0])),
           Int(0),
           Seq(
               Assert(Gtxn[0].xfer_asset() == Int(1088771340)),
                ...
                App.globalPut(Itob(Txn.assets[0]), Gtxn[0].asset_amount()),
                Int(1)
            )
        )
    )
    ...
    program = Cond(
        ...
        [Txn.application_args[0] == Bytes("startFight"), Return(startFight)],
