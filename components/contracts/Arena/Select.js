...
export default class Select extends React.Component { 
        ...
        try {
            ...
            const indexerClient = new algosdk.Indexer(token, 'https://mainnet-algorand.api.purestake.io/idx2', '');
              ...
              if (this.props.activeAddress) {
                ...
                let accountAssets = await indexerClient.lookupAccountAssets(this.props.activeAddress).limit(1000).do();
                ...
                while (numAssets == 1000) {
                    ...
                    accountAssets = await indexerClient.lookupAccountAssets(this.props.activeAddress).limit(1000).nextToken(nextToken).do();
            ...
            this.props.sendDiscordMessage("Arena/Select", props.activeAddress, error)
    ...
    render() {
        ...
        if (this.state.charSelect) {
            return (
                <div>
                    <DisplayChar activeAddress={this.props.activeAddress} contract={this.props.contract} nftId={this.state.charSelect} wallet={this.props.wallet} setNft={(nftId) => this.setState({charSelect: nftId})} select={true} sendErrorMessage={this.sendErrorMessage}/>
                </div>
            )
        }
                    ...
                    ownedNfts.map((nft, index) => {
                        return (
                            <Grid key={index} item xs={6} sm={4} md={3} lg={2} style={{position: "relative"}} >
                                <DisplayChar contract={this.state.contract} style={{position: "absolute"}} nftId={nft} activeAddress={this.props.activeAddress} wallet={this.props.wallet} setNft={(nftId) => this.setState({charSelect: nftId})} sendErrorMessage={this.sendErrorMessage}/>
                            </Grid>
                        )
                    })
                    :
